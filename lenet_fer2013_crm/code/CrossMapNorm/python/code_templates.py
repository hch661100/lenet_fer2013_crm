...
def output_same_shape(out_arg_name, in_arg_name):
    ...
    NVMatrix nv_%(out_arg_name)s(%%(%(out_arg_name)s)s,
     %(out_arg_name)s_dims[1] * %(out_arg_name)s_dims[2] *
     %(out_arg_name)s_dims[3],
     %(out_arg_name)s_dims[0], "%%(class_name)s:nv_%(out_arg_name)s");
    """ % locals()
...
def nv_matrix_create(arg_name):
    ...
    NVMatrix nv_%(arg_name)s(%%(%(arg_name)s)s,
                  %(arg_name)s_dims[1] * %(arg_name)s_dims[2] *
                  %(arg_name)s_dims[3], %(arg_name)s_dims[0],
